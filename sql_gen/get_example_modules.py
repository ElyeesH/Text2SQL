...
class BasicExampleSelector(object):
    def __init__(self):
        ...
        with open("ppl_train_other.json") as f:
    ...
    def get_schemas_and_preresult(self,):
        ...
        # try:
        for table_json in json.load(open("data/spider_data/test_tables.json", "r")):
...
class CosineSimilarExampleSelector(BasicExampleSelector):
    def __init__(self, *args, **kwargs):
        ...
        if os.path.isfile("train_embeddings.json"):
            with open("train_embeddings.json") as f:
                self.train_embeddings = json.load(f)['train_embeddings']
        else:
            self.train_embeddings = self.bert_model.encode(self.train_questions, show_progress_bar=True)
            with open("train_embeddings.json", "w") as f:
                json.dump({'train_embeddings': self.train_embeddings}, f)
